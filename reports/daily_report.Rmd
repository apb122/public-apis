...
status_summary <- data.frame(
  ...
  Status = c(
    ifelse(all_data$astronomy$status == "success", "Success", "Failed"),
    ifelse(all_data$weather$status == "success", "Success", "Failed"),
    ifelse(all_data$crypto$status == "success", "Success", "Failed"),
    ifelse(all_data$time$new_york$status == "success", "Success", "Failed"),
    ifelse(all_data$time$london$status == "success", "Success", "Failed"),
    ifelse(all_data$time$tokyo$status == "success", "Success", "Failed"),
    ifelse(all_data$facts$status == "success", "Success", "Failed"),
    ifelse(all_data$holidays$status == "success", "Success", "Failed"),
    ifelse(all_data$news$hacker_news$status == "success", "Success", "Failed"),
    ifelse(all_data$news$reddit$status == "success", "Success", "Failed"),
    ifelse(all_data$news$wikipedia$status == "success", "Success", "Failed"),
    ifelse(all_data$news$quote$status == "success", "Success", "Failed"),
    ifelse(all_data$news$word$status == "success", "Success", "Failed")
  ),
  `Timestamp` = c(
    format(all_data$astronomy$timestamp, "%H:%M:%S"),
    format(all_data$weather$timestamp, "%H:%M:%S"),
    format(all_data$crypto$timestamp, "%H:%M:%S"),
    format(all_data$time$new_york$timestamp, "%H:%M:%S"),
    format(all_data$time$london$timestamp, "%H:%M:%S"),
    format(all_data$time$tokyo$timestamp, "%H:%M:%S"),
    format(all_data$facts$timestamp, "%H:%M:%S"),
    format(all_data$holidays$timestamp, "%H:%M:%S"),
    format(all_data$news$hacker_news$timestamp, "%H:%M:%S"),
    format(all_data$news$reddit$timestamp, "%H:%M:%S"),
    format(all_data$news$wikipedia$timestamp, "%H:%M:%S"),
    format(all_data$news$quote$timestamp, "%H:%M:%S"),
    format(all_data$news$word$timestamp, "%H:%M:%S")
  ),
  check.names = FALSE
)
